...
const isItPrime = n => {
  ...
  const sqareRootOfN = Math.sqrt(n);
  for(let i = 2; i <= sqareRootOfN; i++)
...
const createMatrix = (n, fill) => {
  ...
  return Array(n).fill(fill,0).map(() => Array(n).fill(fill));
};
...
const areWeCovered = (staff, day) => {
  ...
  return  (staff.filter(x => x.rota.find(rotaDay => rotaDay===day))).length > 2;
};
